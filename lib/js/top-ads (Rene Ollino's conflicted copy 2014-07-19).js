...
$(document).ready(function(){
	...
	// setTimeout(function() {
		...
		var topAd = $("#top-ad-pages");
		var topAdOffset =topAd.offset();
		var topAdH = topAd.height();
		var topAdW = topAd.width();
		var noSlide_1_top = topAdOffset.top;
		var noSlide_1_right = topAdOffset.left + topAdW;
		var noSlide_1_bottom = topAdOffset.top + topAdH;
		var noSlide_1_left = topAdOffset.left;
		console.log("pupa:" + noSlide_1_bottom);

	    function handleMouseMove(event) {
	        event = event || window.event; // IE-ism
	        // event.clientX and event.clientY contain the mouse position
	        if ((event.clientX > noSlide_1_left && event.clientX < noSlide_1_right) && (event.clientY > noSlide_1_top && event.clientY < noSlide_1_bottom)) {
